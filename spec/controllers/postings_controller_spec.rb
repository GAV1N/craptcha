require 'spec_helper'

# This spec was generated by rspec-rails when you ran the scaffold generator.
# It demonstrates how one might use RSpec to specify the controller code that
# was generated by Rails when you ran the scaffold generator.
#
# It assumes that the implementation code is generated by the rails scaffold
# generator.  If you are using any extension libraries to generate different
# controller code, this generated spec may or may not pass.
#
# It only uses APIs available in rails and/or rspec-rails.  There are a number
# of tools you can use to make these specs even more expressive, but we're
# sticking to rails and rspec-rails APIs to keep things simple and stable.
#
# Compared to earlier versions of this generator, there is very limited use of
# stubs and message expectations in this spec.  Stubs are only used when there
# is no simpler way to get a handle on the object needed for the example.
# Message expectations are only used when there is no simpler way to specify
# that an instance is receiving a specific message.

describe PostingsController do

  # This should return the minimal set of attributes required to create a valid
  # Posting. As you add validations to Posting, be sure to
  # update the return value of this method accordingly.
  def valid_attributes
    {}
  end

  describe "GET index" do
    it "assigns all postings as @postings" do
      posting = Posting.create! valid_attributes
      get :index
      assigns(:postings).should eq([posting])
    end
  end

  describe "GET show" do
    it "assigns the requested posting as @posting" do
      posting = Posting.create! valid_attributes
      get :show, :id => posting.id.to_s
      assigns(:posting).should eq(posting)
    end
  end

  describe "GET new" do
    it "assigns a new posting as @posting" do
      get :new
      assigns(:posting).should be_a_new(Posting)
    end
  end

  describe "GET edit" do
    it "assigns the requested posting as @posting" do
      posting = Posting.create! valid_attributes
      get :edit, :id => posting.id.to_s
      assigns(:posting).should eq(posting)
    end
  end

  describe "POST create" do
    describe "with valid params" do
      it "creates a new Posting" do
        expect {
          post :create, :posting => valid_attributes
        }.to change(Posting, :count).by(1)
      end

      it "assigns a newly created posting as @posting" do
        post :create, :posting => valid_attributes
        assigns(:posting).should be_a(Posting)
        assigns(:posting).should be_persisted
      end

      it "redirects to the created posting" do
        post :create, :posting => valid_attributes
        response.should redirect_to(Posting.last)
      end
    end

    describe "with invalid params" do
      it "assigns a newly created but unsaved posting as @posting" do
        # Trigger the behavior that occurs when invalid params are submitted
        Posting.any_instance.stub(:save).and_return(false)
        post :create, :posting => {}
        assigns(:posting).should be_a_new(Posting)
      end

      it "re-renders the 'new' template" do
        # Trigger the behavior that occurs when invalid params are submitted
        Posting.any_instance.stub(:save).and_return(false)
        post :create, :posting => {}
        response.should render_template("new")
      end
    end
  end

  describe "PUT update" do
    describe "with valid params" do
      it "updates the requested posting" do
        posting = Posting.create! valid_attributes
        # Assuming there are no other postings in the database, this
        # specifies that the Posting created on the previous line
        # receives the :update_attributes message with whatever params are
        # submitted in the request.
        Posting.any_instance.should_receive(:update_attributes).with({'these' => 'params'})
        put :update, :id => posting.id, :posting => {'these' => 'params'}
      end

      it "assigns the requested posting as @posting" do
        posting = Posting.create! valid_attributes
        put :update, :id => posting.id, :posting => valid_attributes
        assigns(:posting).should eq(posting)
      end

      it "redirects to the posting" do
        posting = Posting.create! valid_attributes
        put :update, :id => posting.id, :posting => valid_attributes
        response.should redirect_to(posting)
      end
    end

    describe "with invalid params" do
      it "assigns the posting as @posting" do
        posting = Posting.create! valid_attributes
        # Trigger the behavior that occurs when invalid params are submitted
        Posting.any_instance.stub(:save).and_return(false)
        put :update, :id => posting.id.to_s, :posting => {}
        assigns(:posting).should eq(posting)
      end

      it "re-renders the 'edit' template" do
        posting = Posting.create! valid_attributes
        # Trigger the behavior that occurs when invalid params are submitted
        Posting.any_instance.stub(:save).and_return(false)
        put :update, :id => posting.id.to_s, :posting => {}
        response.should render_template("edit")
      end
    end
  end

  describe "DELETE destroy" do
    it "destroys the requested posting" do
      posting = Posting.create! valid_attributes
      expect {
        delete :destroy, :id => posting.id.to_s
      }.to change(Posting, :count).by(-1)
    end

    it "redirects to the postings list" do
      posting = Posting.create! valid_attributes
      delete :destroy, :id => posting.id.to_s
      response.should redirect_to(postings_url)
    end
  end

end
